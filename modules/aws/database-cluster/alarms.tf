# Per-instance CPU alarm published to an existing SNS topic in the account.
...
resource "aws_cloudwatch_metric_alarm" "cpu_high" {
  count = var.cpu_alarm_topic_name != "" ? var.vm_count : 0

  alarm_name          = "${var.env_prefix}-instance-${count.index}-cpu-high"
  alarm_description   = "Average CPU above 90% for 15 minutes on ${var.env_prefix}-instance-${count.index}"
  namespace           = "AWS/EC2"
